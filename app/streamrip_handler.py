...
class StreamripHandler:
    ...
    def __init__(self, config_path: str, music_dir: str):
        ...
        self.completed_jobs: deque[DownloadJob] = deque(maxlen=20)
    ...
    async def search(self, query: str, limit: int = 10, media_type: str = "album") -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception("Error during Qobuz search via Streamrip client")
            return []
        finally:
    ...
    async def get_album_tracks(self, album_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.exception(f"Error fetching album tracks for {album_id}")
            return None
        finally:
            if client and client.session:
                await client.session.close()
        ...
        while True:
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    job.status = "failed"
                    job.error = process.stderr.strip() or process.stdout.strip() or "Unknown error"
