# ── Stage 1: Build & Optimize Python Dependencies (Alpine) ───────────────────
...
RUN set -eux; \
    ...
    # Trim bulky files from dist-info (RECORD, license texts) but keep METADATA for importlib.metadata
    find /install -type f \( -name "RECORD" -o -name "INSTALLER" -o -name "REQUESTED" -o -name "direct_url.json" \) -delete 2>/dev/null || true; \
...
# ── Stage 2: Final Ultra-Lean Runtime Image ──────────────────────────────────
